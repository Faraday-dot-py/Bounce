#!/bin/bash
#SBATCH --job-name=bounce-token-model-v23
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v23-%j.log
...
# v23: v23 (v19 curriculum + --velocity-readout) plus --wall-lookahead (wall penetration and one-step-lookahead penetration features, TokenFreeDynamics). Wall-bounce diagnosis in docs/debugging/experiment-log.md.
...
python -m model.token_train \
  ...
  --velocity-readout --wall-lookahead --wall-head --lr 1e-3 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h24_v23.pt
