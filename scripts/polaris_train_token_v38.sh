#!/bin/bash
#SBATCH --job-name=bounce-token-model-v38
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v38-%j.log
...
# v38: continue soup B (v30-v37 lineage average, checkpoints/token_model_soup_b.pt) for 3000 batches at 40-step unrolls with --contact-weight 3.0 (token_state_loss up-weighted 3x for tokens in or entering pair contact; the contact-context diagnosis found pair contact carries ~64% of one-step position error) and --speed-weight 0.3. v39 is the control without it.
...
python -m model.token_train \
  ...
  --n 20 --horizon 44 \
  --free-rollout --stepwise-curriculum --curriculum-min-steps 40 --curriculum-max-steps 40 --init-checkpoint checkpoints/token_model_soup_b.pt \
  ...
  --velocity-readout --position-refine --ball-split --wall-lookahead --wall-head --pair-impulse --speed-weight 0.3 --contact-weight 3.0 --lr 3e-4 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h44_v38.pt
