#!/bin/bash
#SBATCH --job-name=bounce-token-model-v37
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v37-%j.log
...
# v37: continue v33 for 3000 more batches at 40-step unrolls with --speed-weight 0.3 and --state-vel-weight 0.5 (velocity term of token_state_loss raised from 0.1; compare with v37).
...
python -m model.token_train \
  ...
  --velocity-readout --position-refine --ball-split --wall-lookahead --wall-head --pair-impulse --speed-weight 0.3 --state-vel-weight 0.5 --lr 3e-4 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h44_v37.pt
