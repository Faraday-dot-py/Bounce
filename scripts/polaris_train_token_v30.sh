#!/bin/bash
#SBATCH --job-name=bounce-token-model-v30
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v30-%j.log
...
# v30: v29 plus --speed-weight 0.1 (|v| magnitude loss); fine-tune v25 (all options incl. --position-refine) for 1500 batches at 40-step unrolls (lr 3e-4, 50% replay of shorter) on the h44 dataset. v26 (from scratch to 26 steps) hurt short-horizon accuracy; the energy audit fine-tune from v24 helped (docs/debugging/experiment-log.md).
...
python -m model.token_train \
  ...
  --checkpoint checkpoints/token_model_h44_v30.pt
