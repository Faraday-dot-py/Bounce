#!/bin/bash
#SBATCH --job-name=bounce-token-model-v29
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v29-%j.log
...
# v29: fine-tune v25 (all options incl. --position-refine) for 1500 batches at 40-step unrolls (lr 3e-4, 50% replay of shorter) on the h44 dataset. v26 (from scratch to 26 steps) hurt short-horizon accuracy; the energy audit fine-tune from v24 helped (docs/debugging/experiment-log.md).
...
python -m model.token_train \
  ...
  --n 20 --horizon 44 \
  --free-rollout --stepwise-curriculum --curriculum-min-steps 40 --curriculum-max-steps 40 --init-checkpoint checkpoints/token_model_h24_v25.pt \
  ...
  --velocity-readout --position-refine --wall-lookahead --wall-head --pair-impulse --lr 3e-4 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h44_v29.pt
