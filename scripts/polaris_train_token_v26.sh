#!/bin/bash
#SBATCH --job-name=bounce-token-model-v26
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v26-%j.log
...
# v26: v25 (all options incl. --position-refine) with 40-step unrolls on the h44 dataset (job 2886), 3 chunks per stage max. Energy audit: collapse comes from states beyond the 20-step training horizon (docs/debugging/experiment-log.md).
...
python -m model.token_train \
  ...
  --velocity-readout --position-refine --wall-lookahead --wall-head --pair-impulse --lr 1e-3 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h44_v26.pt
