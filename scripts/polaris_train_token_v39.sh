#!/bin/bash
#SBATCH --job-name=bounce-token-model-v39
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v39-%j.log
...
# v39: control for v38: continue soup B for 3000 batches at 40-step unrolls with --speed-weight 0.3 and no contact weighting.
...
python -m model.token_train \
  ...
  --velocity-readout --position-refine --ball-split --wall-lookahead --wall-head --pair-impulse --speed-weight 0.3 --lr 3e-4 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h44_v39.pt
