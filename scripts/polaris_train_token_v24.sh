#!/bin/bash
#SBATCH --job-name=bounce-token-model-v24
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v24-%j.log
...
# v24: v23 (v19 curriculum + --velocity-readout --wall-lookahead --wall-head) plus --pair-impulse (explicit antisymmetric sum-aggregated pair impulse, TokenFreeDynamics). Pair-collision diagnosis in docs/debugging/experiment-log.md.
...
python -m model.token_train \
  ...
  --velocity-readout --wall-lookahead --wall-head --pair-impulse --lr 1e-3 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h24_v24.pt
