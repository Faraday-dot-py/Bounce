#!/bin/bash
#SBATCH --job-name=bounce-token-model-v28
#SBATCH --partition=gpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=bounce-token-model-v28-%j.log
...
# v28: v25 (all options incl. --position-refine) plus --ball-split (overlap-resolving init detection, model/token_split.py).
...
python -m model.token_train \
  ...
  --velocity-readout --position-refine --ball-split --wall-lookahead --wall-head --pair-impulse --lr 1e-3 \
  --seed 4738 --log-every 0 \
  --checkpoint checkpoints/token_model_h24_v28.pt
