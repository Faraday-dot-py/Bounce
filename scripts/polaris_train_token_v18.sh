...
# ramps 4 -> 16 steps over 5 epochs; 10 epochs total because per-epoch cost
# scales with unroll length (50 epochs at 24 steps did not fit the limit).
# Primary criteria: mean position error
...
python -m model.token_train \
  ...
  --n 20 --horizon 16 --epochs 10 \
  --free-rollout --horizon-start 4 --horizon-ramp 5 \
